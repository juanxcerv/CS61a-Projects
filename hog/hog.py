...
def is_perfect_piggy(turn_score):
    ...
    if (turn_score == 1):
        return False
    if (is_square(turn_score) or is_cubed(turn_score)):
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while(score0 < goal and score1 < goal):
        ...
        if (player == 0):
            ...
            player = other(player)
            curr = take_turn(num_rolls0, score1, dice)
            score0 += curr
            if (is_perfect_piggy(curr)):
            ...
            player = other(player)
            curr1 = take_turn(num_rolls1, score0, dice)
            score1 += curr1
            if (is_perfect_piggy(curr1)):
                dice_swapped = not dice_swapped
            if is_swap(score0, score1):
                temp = score0
                score0 = score1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    assert (num_samples > 0), "Must roll at least once!"
    minim = 0
    j = 0
    for i in range(1,11):
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    stolen = free_bacon(opponent_score)
    if is_prime(stolen):
        stolen = next_prime(stolen)
    if stolen >= margin:
        return 0
    else:
        return num_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 11
    stolen = free_bacon(opponent_score)
    if is_swap(stolen + score, opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    "*** REPLACE THIS LINE ***"
    if score < 20 and opponent_score > 80:
    # END PROBLEM 12
