...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    arr = [[loc,distance(location, loc)] for loc in centroids]
    closest = 1000000
    clocation = []
    for a in arr:
        if a[1] < closest:
...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    def centroid(loc):
        return [find_closest(restaurant_location(loc), centroids)]
    lst = [[centroid(loc), loc] for loc in restaurants]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(cluster) for cluster in grouped]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xx = sum([(x - mean_xs) ** 2 for x in xs])
    s_yy = sum([(y - mean_ys) ** 2 for y in ys])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for fn in feature_fns:
        s.append(find_predictor(user, reviewed, fn))
    max = 0
    best_fn = None
    for pair in s:
        if pair[1] > max:
